...
var app = express();
var pg = require('pg');
var client = new pg.Client(process.env.DATABASE_URL || 'postgresql://mark@localhost/linkshortener');
...
app.get('/:link_id', function(req, res) {
    ...
    client.query('SELECT url, random FROM links WHERE id = ' + id, function(err, result) {
        if (err) {
            res.send(500);
            return;
        }
